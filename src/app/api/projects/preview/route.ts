...
export async function POST(req: Request) {
  ...
  try {
    ...
    return Response.json({
      ...
      fields: res.schema.fields.map((f) => ({ id: f.id, label: f.label, source: f.source, type: f.type, role: f.role, semantic: f.semantic ?? null, derived: Boolean(f.derived), samples: sheet.stats.find((s) => s.header === f.source)?.samples.slice(0, 3) ?? [] })),
