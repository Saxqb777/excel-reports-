    ...
    <main className="flex min-h-screen items-center justify-center bg-bg px-6">
      <form onSubmit={submit} className="w-full max-w-sm border border-line bg-bg-elev p-6">
        <div className="mb-4 flex items-center gap-2.5">
          <span className="num flex h-5 min-w-5 items-center justify-center px-1 text-[10px] font-semibold" style={{ background: primary, color: "#fff" }}>{name.slice(0, 2).toUpperCase()}</span>
          <div><div className="text-[13px] font-semibold text-ink">{name}</div>{clientName && <div className="text-[11px] text-ink-3">{clientName}</div>}</div>
        </div>
        ...
        {error && <div className="mt-2 text-[12px] text-neg">{error}</div>}
        <button type="submit" className="btn btn-accent mt-4 w-full" disabled={busy || !password}>{busy ? "Checking…" : "Open dashboard"}</button>
        <p className="mt-3 text-[11px] text-ink-3">This dashboard is shared privately. Ask the owner for the password.</p>
      </form>
    </main>
  );
}
