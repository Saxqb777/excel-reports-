    ...
    <div className="fixed inset-0 z-[65]" onClick={onClose}>
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-md flex-col border-l border-line bg-bg shadow-[var(--shadow-pop)]" onClick={(e) => e.stopPropagation()} aria-label="Upload history">
        ...
        <div className="min-h-0 flex-1 overflow-auto">
          {uploads === null ? <div className="p-4 text-[12px] text-ink-3">Loading…</div> : uploads.length === 0 ? <div className="p-4 text-[12px] text-ink-3">No uploads yet.</div> : (
                ...
                return (
                  <li key={u.id} className={`px-4 py-3 ${current ? "bg-[var(--accent-wash)]" : ""}`}>
                    <div className="flex items-baseline justify-between gap-3">
                      <span className="num text-[13px] font-medium text-ink">v{u.versionNo} {current && <span className="label ml-1 text-accent">live</span>}</span>
                      <span className="num text-[11px] text-ink-3">{formatDateTime(u.uploadedAt)}</span>
                    </div>
                    <div className="num mt-0.5 text-[11.5px] text-ink-2">{u.rowCount} rows · {u.excludedCount} excluded · by {u.uploadedBy}</div>
                    <div className="num mt-0.5 truncate text-[11px] text-ink-3" title={u.fileName}>{u.fileName} · {(u.fileSize / 1024).toFixed(1)} KB</div>
                    <div className="mt-2 flex gap-2">
                      {!current && <button type="button" className="btn h-6 px-2 py-0 text-[11px]" disabled={busy === u.id} onClick={() => restore(u)}>{busy === u.id ? "Restoring…" : "Restore this version"}</button>}
                      <a className="btn h-6 px-2 py-0 text-[11px] leading-6" href={`/api/projects/${projectId}/uploads/${u.id}/file`}>Download file</a>
                    </div>
                  </li>
                );
              })}
        ...
        <div className="border-t border-line px-4 py-2 text-[11px] text-ink-3">Restoring makes an older version live again. Nothing is deleted.</div>
      </aside>
    </div>
  );
}
