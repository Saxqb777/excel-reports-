...
- Audience for the Agthia dashboard: the user is on the transporter side, the company is NXN (spelled exactly so, not "Nexon"); the dashboard is shown to NXN's CEO, who wants to see his company name. The live project is named "Agthia × NXN" with monogram "NXN" and no separate client name. "Awaiting client" means waiting for Agthia, "Not yet quoted" means NXN has not quoted. Insights are worded frankly. It must look clean: no operational notes, no personal names.
...
- The user sends the working dashboard link (`/p/agthia`) to the CEO on purpose, so it carries the same title and preview card as the share link and `src/proxy.ts` also rewrites `/p/<slug>` for crawlers. Anyone with that link can edit and delete: sign-in remains roadmap item 1.
